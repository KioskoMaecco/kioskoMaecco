...
namespace kioskotem.viaje
{
    public partial class comprobante : System.Web.UI.Page
    {
        ...
        protected void dtgnominas_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Select")
                {
                    ...
                    Session["ruta"] = "dviaje/" + sa + ".pdf";
                    String path = Server.MapPath("../dviaje") + "\\" + sa + ".pdf";
                    String path2 = "../dviaje/" + sa + ".pdf";
        ...
        private void cargar_grid()
        {
            ...
            try
            {
                Tabla tbEmpresas = Manejador.getEjecutaStoredProcedure1("getviaje", "1|" + Session["idusuario"].ToString() + "|" + inicial + "|" + fin);
                if (tbEmpresas != null)
                {
                    DataTable dtEmpresas = clFunciones.convertToDatatable(tbEmpresas);
                    for (int x = 0; x < dtEmpresas.Rows.Count; x++)
                    {
                        dsEmpresas.Tables[0].Rows.Add(dtEmpresas.Rows[x]["iIdViaje"],
