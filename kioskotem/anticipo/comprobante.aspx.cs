...
namespace kioskotem.anticipo
{
    public partial class comprobante : System.Web.UI.Page
    {
        ...
        protected void dtgnominas_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Select")
                {
                    ...
                    string sa = ((Label)dtgnominas.Rows[id].FindControl("lbldpago")).Text;

                    Session["ruta"] = "danticipo/" + sa + ".pdf";
                    String path = Server.MapPath("../danticipo") + "\\" + sa + ".pdf";
                    String path2 = "../danticipo/" + sa + ".pdf";
        ...
        private void cargar_grid()
        {
            ...
            dsEmpresas.Tables[0].Columns.Add("dpagoant");
            ...
            try
            {
                Tabla tbEmpresas = Manejador.getEjecutaStoredProcedure1("getanticiponomina", "1|" + Session["idusuario"].ToString() + "|" + inicial + "|" + fin);
                if (tbEmpresas != null)
                {
                    DataTable dtEmpresas = clFunciones.convertToDatatable(tbEmpresas);
                    for (int x = 0; x < dtEmpresas.Rows.Count; x++)
                    {
                        dsEmpresas.Tables[0].Rows.Add(dtEmpresas.Rows[x]["iIdAnticipo"],
                                                        dtEmpresas.Rows[x]["documento"],
